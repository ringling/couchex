defmodule MapperTest do
  ...
  should "map tuple list to Map" do
    assert %{"_id" => "id", "_rev" => "rev", "foo" => "bar"} == Mapper.list_to_map([{"_id", "id"}, {"_rev", "rev"}, {"foo", "bar"}])
  end

  should "map tuple list with 2 tuples" do
    assert %{"foo" => "bar", "baz"=>"bozz" } == Mapper.list_to_map([{"foo", "bar"}, {"baz", "bozz"}])
  end

  should "map tuple list to Map 1 level" do
    assert %{"foo" => %{"bar" => "baz"} } == Mapper.list_to_map([{"foo", {"bar", "baz"} }])
  end

  should "map tuple list to Map 2 level" do
    assert %{"foo" => %{"bar" => %{"baz" => "bozz" } } } == Mapper.list_to_map([{"foo", {"bar", {"baz","bozz"} } }])
  end

  should "map tuple list to Map 2 level with tuple list" do
    assert %{"foo" => %{"bar" => [%{"baz" => "bozz" }] } } == Mapper.list_to_map([{"foo", {"bar", [{"baz", "bozz"}] } }])
  end

  should "map tuple list to Map 2 level with list" do
    assert %{"foo" => %{"bar" => %{"baz" => [1,2,3] } } } == Mapper.list_to_map([{"foo", {"bar", {"baz",[1,2,3]} } }])
  end

  should "map tuple list to Map 2 level with empty list" do
    assert %{"foo" => %{"bar" => %{"baz" => [] } } } == Mapper.list_to_map([{"foo", {"bar", {"baz",[]} } }])
  end

  should "map tuple list to Map 2x2" do
    assert %{"a" => %{"b" => %{"c" => "c1" }}, "foo" => %{"bar" => %{"baz" => "bozz" }}} == Mapper.list_to_map([{"a", {"b",{"c","c1"} } },{"foo", {"bar", {"baz","bozz"} } }])
  end

  should "map nested tuple list" do
    resp = [
      {
        [
          {"id", "doc_id_1"},
